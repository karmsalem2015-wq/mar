...
export default function HeroVideoSection({ searchBar }: HeroVideoSectionProps = {}) {
  ...
  const applyVideoTime = useCallback((progress: number, force = false) => {
    ...
    const desiredTime = Math.min(Math.max(progress, 0), 1) * safeDuration;
  ...
  const stepVideoScrub = useCallback((timestamp: number) => {
    ...
    if (rawRafMs > 24) diag.longFrames += 1;

    const target = targetProgressRef.current;
    const current = smoothProgressRef.current;
    const delta = target - current;

    // Scroll only sets the destination. RAF owns the cinematic playhead.
    // The lower mobile response constant keeps touch bursts fluid without lagging.
    ...
    // Do not issue redundant seeks faster than the source frame cadence.
  ...
  const syncStoryToScroll = useCallback(() => {
    ...
    const targetJump = Math.abs(rawVideoProgress - lastDiagTargetRef.current);
    diagRef.current.maxTargetJump = Math.max(diagRef.current.maxTargetJump, targetJump);
  ...
  useEffect(() => {
    if (shouldReduceMotion || videoFailed) return;
    ...
    (window as HeroDiagWindow).__MAR_HERO_DIAG__ = () => {
      const video = videoRef.current;
      let bufferedAhead = 0;
    ...
    return () => {
      ...
      delete (window as HeroDiagWindow).__MAR_HERO_DIAG__;
      if (scrollRafRef.current !== null) window.cancelAnimationFrame(scrollRafRef.current);
      if (scrubRafRef.current !== null) window.cancelAnimationFrame(scrubRafRef.current);
      scrollRafRef.current = null;
      scrubRafRef.current = null;
    };
  }, [mediaVariant, requestScrollSync, shouldReduceMotion, updateSectionMetrics, videoFailed]);

  useEffect(() => {
    if (!mediaVariant || shouldReduceMotion || isVideoReady || videoFailed) return;
    const timeout = window.setTimeout(() => setVideoFailed(true), VIDEO_READY_TIMEOUT_MS);
    return () => window.clearTimeout(timeout);
  }, [isVideoReady, mediaVariant, shouldReduceMotion, videoFailed]);

  if (videoFailed) {
    return <HeroFramesSection searchBar={searchBar} />;
  }

  const activeStop = activeStopIndex >= 0 ? HERO_STORY_STOPS[activeStopIndex] : null;
  ...
  return (
    ...
    >
      <div className="sticky top-0 h-screen h-[100svh] w-full overflow-hidden bg-[#060D1A]">
        ...
        {!isVideoReady && !shouldReduceMotion && (
